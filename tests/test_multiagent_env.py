...
# ---------------------------------------------------------------------------
# Test 1: observation size is exactly 289 (v19: agent grid bumped 4→5 channels)
# ---------------------------------------------------------------------------

def test_agent_obs_size() -> None:
    """Obs vector must be exactly OBS_TOTAL_SIZE (289) floats.
    ...
    assert OBS_TOTAL_SIZE == 289, f"Expected 289, got {OBS_TOTAL_SIZE}"

    env = _make_env(n_agents=5)
    obs, _ = env.reset(seed=0)
    assert obs.shape == (289,), f"Expected obs shape (289,), got {obs.shape}"
