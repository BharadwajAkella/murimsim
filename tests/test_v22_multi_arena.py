...
def test_minion_dies_after_enough_damage():
    ...
    # Cumulative damage ≥ HP kills it
    m.take_damage(0.5, "agent_a")
    m.take_damage(0.5, "agent_b")
    assert m.alive
    killed = m.take_damage(0.6, "agent_a")
    assert killed
    assert not m.alive
...
def test_minion_attack_damage_capped():
    ...
    # Even with strength=2.0 the per-hit cap is MINION_ATTACK_MAX_DAMAGE=0.30
    m.strength = 2.0
    assert m.attack_damage() <= 0.30
...
def test_build_envs_arena_mix_round_robin():
    ...
    assert envs[1]._n_minions == 2 and not envs[1]._enable_boss
    assert envs[2]._world.grid_size == 12
    assert envs[2]._n_minions == 2 and not envs[2]._enable_boss
    assert envs[3]._world.grid_size == 16
